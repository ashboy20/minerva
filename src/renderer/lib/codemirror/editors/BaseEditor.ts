...
export abstract class BaseEditor {
  ...
  protected initialize(): void {
    ...
    const content = this.processInitialContent(this.options.doc || '')
  ...
  setContent(newContent: string): void {
    if (this.view) {
      ...
      const processedContent = this.processContentUpdate(newContent)
      this.view.dispatch({
        changes: { from: 0, to: currentDoc.length, insert: processedContent },
        selection: this.getSelectionAfterContentUpdate(processedContent)
      })
