...
interface UrlState {
  ...
  queryParams: Row[];
  fullUrl: string;
  lastUpdateSource: 'url' | 'pathParams' | 'queryParams' | null;
}
...
const parsePathParams = (path: string): Row[] => {
  ...
  const matches = pathWithoutProtocol.match(pathParamsRegex) || [];
...
const buildUrlFromParams = (baseUrl: string, path: string, pathParams: Row[], queryParams: Row[]): string => {
  ...
  pathParams.forEach(param => {
    if (param.enabled && param.keyValue && param.value) {
      fullUrl = fullUrl.replace(`:${param.keyValue}`, param.value);
    }
  });
  ...
  if (enabledQueryParams.length > 0) {
    const queryString = enabledQueryParams
      .map(p => `${encodeURIComponent(p.keyValue)}=${encodeURIComponent(p.value)}`)
      .join('&');
    fullUrl += `?${queryString}`;
  }
...
const extractBaseAndPath = (fullUrl: string, currentBaseUrl: string = ''): { baseUrl: string; path: string } => {
  // Remove query parameters first
  const [urlWithoutQuery] = fullUrl.split('?');
  
  // If currentBaseUrl exists and URL starts with it, extract path
  if (currentBaseUrl && urlWithoutQuery.startsWith(currentBaseUrl)) {
    return {
      baseUrl: currentBaseUrl,
      path: urlWithoutQuery.replace(currentBaseUrl, '') || '/',
    };
  }
  ...
  return {
    baseUrl: currentBaseUrl,
    path: urlWithoutQuery.startsWith('/') ? urlWithoutQuery : '/' + urlWithoutQuery,
  };
};
...
export const urlSlice = createSlice({
  ...
  reducers: {
    ...
    initializeUrl: (state, action: PayloadAction<{ baseUrl: string; path: string; pathParams?: Row[]; queryParams?: Row[] }>) => {
      const { baseUrl, path, pathParams = [], queryParams = [] } = action.payload;
      state.baseUrl = baseUrl;
      state.path = path;
      state.pathParams = pathParams;
      state.queryParams = queryParams;
      state.fullUrl = buildUrlFromParams(baseUrl, path, pathParams, queryParams);
    ...
    updateFromUrl: (state, action: PayloadAction<string>) => {
      ...
      const { baseUrl, path } = extractBaseAndPath(newUrl, state.baseUrl);
      state.baseUrl = baseUrl;
      state.path = path;
    ...
    updatePathParams: (state, action: PayloadAction<Row[]>) => {
      ...
      const hasChanged = JSON.stringify(state.pathParams) !== JSON.stringify(newPathParams);
      if (!hasChanged) {
        return;
      }
      ...
      const pathWithoutProtocol = newPath.replace(/^https?:\/\/[^\/]*/, '');
      const pathParamsRegex = /\:[a-zA-Z0-9_]+/g;
      const pathParamsLocated = pathWithoutProtocol.match(pathParamsRegex);
      
      if (pathParamsLocated && state.pathParams.length > 0) {
        // Replace each path parameter in order
        let updatedPath = newPath;
        pathParamsLocated.forEach((oldParam, index) => {
          if (state.pathParams[index] && state.pathParams[index].keyValue) {
            const newParam = `:${state.pathParams[index].keyValue}`;
            // Replace the specific occurrence by position
            updatedPath = updatedPath.replace(oldParam, newParam);
          }
        });
      ...
      state.fullUrl = buildUrlFromParams(state.baseUrl, state.path, state.pathParams, state.queryParams);
    ...
    updateQueryParams: (state, action: PayloadAction<Row[]>) => {
      ...
      // Check if parameters actually changed to prevent unnecessary updates
      const hasChanged = JSON.stringify(state.queryParams) !== JSON.stringify(newQueryParams);
      if (!hasChanged) {
        return;
      }
